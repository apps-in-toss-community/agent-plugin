...
1. **dev 명령 확인**: `package.json`의 `scripts.dev`를 읽어 실제 사용할 명령을 확인한다(일반적으로 `vite` 또는 `vite --port <n>`).

2. **기동 여부 확인**: Vite 기본 포트(5173)가 응답하는지 확인한다:

   ```bash
   # 포트가 이미 열려 있으면 dev 서버가 기동 중
...
4. **URL 출력**: Vite가 stdout에 `Local: http://localhost:<port>/`를 출력한다. 에이전트는 이 줄을 파싱해 포트를 확인하고 사용자에게 URL을 명시적으로 알린다:
   ...
   포트가 `package.json`의 `dev` 스크립트에 명시돼 있으면 그 값을 쓴다. 명시되지 않은 경우 Vite 기본값 5173을 사용한다.
...
`start_debug({mode})`를 호출해 런타임에 결정한다. 환경 3·4(relay)는 한 데몬에서
warm swap으로 자유롭게 오갈 수 있다(Claude Code 재구동·MCP 재핸드셰이크 불필요).

| `mode` 값 | 환경 | 특이사항 |
|---|---|---|
| `local-browser` | 환경 1 — mock Chromium panel / 브라우저 CDP | 기본값. panel 모드와 CDP 직접 연결 모두 이 mode 사용 |
| `relay-sandbox` | 환경 2 — 실기기 PWA (외부 relay) | mock SDK; `dev:phone:cdp` 스크립트 + `tunnel:{cdp:true}` 필요. **기본 데몬(`--target=relay`)에서는 동적 전환 불가** — 아래 주의사항 참조 |
...
> **`relay-sandbox`(환경 2) 진입 — 기본 데몬에서는 불가, dual-connection 데몬 필요**:
...
>
> plugin manifest가 등록하는 기본 데몬(`npx -y @ait-co/devtools devtools-mcp`)은 `--target` 플래그
> 없이 기동되므로 `--target=relay`(single-connection)로 동작한다. 이 single-connection 데몬은
> `start_debug({mode:'relay-sandbox'})` 호출 시 **동적 전환을 거부**한다("이 세션은 단일 연결만
> 보유합니다 — relay-sandbox로 동적 전환할 수 없습니다. MCP 서버를 relay-sandbox 모드로
> 재시작하세요"라는 에러가 반환된다). relay 주소 유무와 무관하게 이 거부가 먼저 발생한다.
>
> `relay-sandbox` 진입이 가능한 데몬은 `--target=mobile`(dual-connection)로 기동된 경우뿐이다.
> 단, `--target=mobile` 데몬은 부팅 시 `AIT_RELAY_BASE_URL` 또는 `.ait_urls` 파일이 없으면
> 즉시 hard-fail하므로 `/ait setup-phone-preview`로 터널·relay를 먼저 배선해야 한다.
>
> **현재 알려진 한계**: 환경 2 MCP attach를 기본 plugin 데몬에서 in-place로 진입하는 경로는
> 지원되지 않는다. 환경 2 실기기 WebKit 엔진은 `/ait setup-phone-preview`로 배선한 뒤
> 해당 skill의 안내에 따라 relay-sandbox 가능 데몬을 별도 구성해야 한다(5-A 참조).
>
> `relay-sandbox`로 진입을 시도했다가 위 에러("MCP 서버를 relay-sandbox 모드로 재시작하세요")를
> 받은 경우: 현재 데몬이 single-connection(`--target=relay`)으로 기동된 상태이며,
> relay를 먼저 배선(`/ait setup-phone-preview`)한 뒤 dual-connection 데몬으로 재시작해야 한다.
> plugin.json의 기본 등록은 `--target=relay`로 유지되므로 `/mcp` 설정에서 별도로 조정한다.
...
1. **`start_debug`** 도구를 먼저 호출해 relay mode를 설정한다:
   - 환경 3: `start_debug({mode: 'relay-staging'})`
...
- ❌ 기본 plugin 데몬(`--target=relay`, single-connection)에서 `relay-sandbox` 진입 기대.
  단일 연결 데몬은 `relay-sandbox` 동적 전환을 거부한다 — relay 주소 유무와 무관하게 하드 에러.
  환경 2 MCP attach는 dual-connection(`--target=mobile`) 데몬이 필요하며, 그 데몬도
  `/ait setup-phone-preview`로 터널·relay를 먼저 배선한 뒤에야 부팅 가능하다(5-A 주의사항 참조).
