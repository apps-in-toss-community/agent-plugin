...
description: |
  ...
  (console / network). Environments 2/3 (on-device intoss-private candidate /
  live bundle): the `ait-devtools` MCP server — registered by this plugin and
  always running — issues a QR attach URL via `build_attach_url`; once the phone
  scans it and the relay attaches, attach-dependent tools register dynamically in
  the same session. `/ait debug` branches by what it observes and prints the
  right path. Triggered by `/ait debug`.
argument-hint: ''
---
...
`/ait debug`는 미니앱을 **세 환경**에서 디버깅하는 경로를 안내한다. 한 명령이
관찰 결과에 따라 환경을 분기한다 (umbrella `CLAUDE.md` §1.1 환경 3종 모델):

| 환경 | 실행 면 | 이 skill의 경로 |
|---|---|---|
| 1. 로컬 브라우저 | desktop Chromium + mock SDK + Panel | 2-A/2-B/3 — panel · `window.__ait` · 브라우저 DevTools |
| 2. intoss-private relay dev | 실기기 토스 앱 WebView(dogfood) + CDP relay | 5 — `build_attach_url` QR로 relay attach |
| 3. intoss live relay debug | 실기기 토스 앱 WebView(LIVE, 검수 통과) + CDP relay | 5 — 동일 relay, LIVE attach (read-only) |
...
- **환경 2·3**은 `ait-devtools` MCP 서버로 닿는다. 이 서버는 plugin이 manifest에
...
1. `ait-devtools` 서버의 **`build_attach_url`** 도구를 호출한다 (scheme URL 전달).
   ...
   QR을 터미널에 렌더**한다.
2. 사용자가 **폰 카메라로 그 QR을 스캔**한다 — 이게 환경 2·3의 단일 진입 경로다.
...
- **환경 1에서 재현·진단 끝** → 수정은 에이전트의 일반 편집 흐름으로. 폰에서만
  나는 회귀라면 환경 2로: `/ait deploy`로 candidate를 만들고 5-B의 QR attach.
- **candidate scheme URL이 아직 없음** → `/ait setup-bundle` → `/ait register` →
  `/ait deploy`로 candidate를 만든 뒤 다시 `/ait debug`.
- **환경 3(LIVE) 운영 관측** → `/ait status`, `/ait logs`로 콘솔 상태도 함께 확인.

## 참고

- 짝 skill: `inject-devtools` (panel 설정), `setup-phone-preview` (실기기 미리보기 tunnel).
- 환경 3종 × fidelity 설계 정본: umbrella `meta/three-environments-fidelity.md` (§1 환경 모델, §5 동적 도구 등록, §7 CDP 단일 transport).
