...
description: |
  ...
  Environment 3 (on-device intoss-private candidate): call
...
**`MCP_ENV`** (deprecated back-compat): 구버전 별칭이다. 신규 환경 진입에는 `start_debug`를 쓴다 — `MCP_ENV`를 서버 기동 전에
...
4. **`build_attach_url()`** 도구를 호출한다(scheme URL 불필요 — 환경 2는
   `.ait_urls`에서 읽은 터널을 사용, 환경 3의 scheme URL과 다름). 서버가
...
3. 사용자가 **폰 카메라로 QR을 스캔**한다 — 이게 환경 3의 단일 진입 경로다.
