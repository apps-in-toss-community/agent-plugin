...
관찰 결과에 따라 환경을 분기한다 (umbrella `CLAUDE.md` §1.1 환경 3겹 모델):

| 환경 | 실행 면 | 이 skill의 경로 |
|---|---|---|
| 1. 로컬 브라우저 | desktop Chromium + mock SDK + Panel | 2-A/2-B/3 — panel · `window.__ait` · 브라우저 DevTools |
| 2. AITC Sandbox App (PWA) | 실기기 Safari/WebKit + installable PWA(`devtools.aitc.dev/launcher/`) + cloudflared 터널 | 5 — `start_attach({mode:'relay-sandbox'})` 1호출 QR attach (mock SDK; CDP는 실 WebKit; `setup-phone-preview`로 `dev:phone:cdp` 스크립트 + CDP relay 배선 선행, 이 skill이 dev 서버 기동 자동화) |
...
- **환경 2·3**은 `ait-devtools` MCP 서버로 닿는다. 이 서버는 plugin이 manifest에
  등록해 **상시 기동**되므로, `/ait debug`는 새 서버를 띄우지 않고 **`start_attach({mode})`로
  환경 전환과 QR attach 경로 발급을 한 호출로** 처리한다(아래 5). 환경 2(`relay-sandbox` mode)는
...
#### `start_debug(mode)` / `start_attach(mode)` — 런타임 환경 전환과 attach
...
> → `pnpm dev:phone:cdp` → `start_attach({mode:'relay-sandbox'})`이다(5-C relay-sandbox 분기).
>
...
**`MCP_ENV`** (deprecated back-compat): 구버전 별칭이다. 신규 환경 진입에는 `start_debug` 또는 `start_attach`를 쓴다 — `MCP_ENV`를 서버 기동 전에
명시하는 방식은 정본 진입 경로가 아니다.

attach까지 한 번에 처리하려면 바로 아래 5-B·5-C 순서를 따른다(`start_attach`가 환경 전환+QR 발급을 1호출로).
...
candidate scheme URL이 없으면 먼저 `/ait deploy`를 실행하세요 — 빌드·인증·업로드를 자동 처리하고 scheme URL을 돌려줍니다. URL을 받으면 5-C로 돌아와 `start_attach`에 전달합니다.
...
1. **`start_attach({mode: 'relay-staging', scheme_url, projectRoot})`** 도구를 호출한다 (5-B에서 얻은 scheme URL을 `scheme_url`로 전달).
...
- ❌ 환경 2 relay 배선 없이 `relay-sandbox` 진입 기대. 기본 데몬은 런타임에
  ...
  한다. 주소가 없으면 `start_attach`가 relay 주소 미설정 에러를 돌려준다 — 데몬
...
- **환경 1에서 재현·진단 끝** → 수정은 에이전트의 일반 편집 흐름으로. 브라우저에서
  ...
  `/ait deploy`로 scheme URL을 받아 5-C의 `start_attach({mode:'relay-staging', scheme_url})`으로 QR attach.
