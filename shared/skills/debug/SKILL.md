...
description: |
  ...
  production `.ait` bundle (CDP relay MCP) is a separate, in-progress
  surface and is described at the end as the next step, not invoked here.
  Triggered by `/ait debug`.
argument-hint: ''
---
...
on-device 경로(CDP relay 기반)는 별도의 진행 중 surface다 — 이 skill 끝의
"다음 단계"에서 안내만 하고, 여기서 실행하지는 않는다.
...
## 다음 단계: production 번들 on-device 디버깅 (진행 중)
...
gate의 순수 판정 로직은 이미 devtools에 들어가 있다(`@ait-co/devtools/in-app`의
`evaluateDebugGate`). relay transport / tunnel / 보안 토폴로지 / MCP 서버 부착은
설계 확정 후 단계적으로 들어온다. 진행 상황은 아래 참고 링크의 spec과 umbrella
TODO를 따른다. on-device 경로가 활성화되면 이 skill이 그 흐름까지 안내하도록
확장한다.
...
- ❌ production 번들에 debug surface가 "이미 동작한다"고 단정. on-device 경로는
  설계/구현 중 — 지금 동작하는 건 브라우저 디버깅이다.
...
- on-device debug 설계: devtools repo `docs/superpowers/specs/2026-05-18-in-app-debug-mcp.md`
